...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "key points, problems and fixes, tech stack, link. Use it after search_projects, "
...
class ChatbotService:
    ...
    async def _context(self, message: str, history: list[dict]) -> str:
        ...
        if info:
            ...
            if info.other_projects:
                lines.append("그 외 프로젝트(이름만 공개): " + " / ".join(str(one) for one in info.other_projects))
    ...
    def _project_context(project) -> str:
        ...
        for label, items in (("핵심", project.highlights), ("문제와 개선", project.improvements)):
            if items:
        ...
        if project.metrics:
            rows.append("지표: " + ", ".join(f"{m.get('label')} {m.get('value')}" for m in project.metrics))
        if project.url:
