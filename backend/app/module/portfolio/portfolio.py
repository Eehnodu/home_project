...
class PortfolioInfo(Base):
    ...
    # 공개 화면 구성. header(상단 헤더 + 한 페이지 스크롤) | sidebar(왼쪽 메뉴 + 한 페이지 스크롤)
    ...
    # 상세로 올리지 않은 프로젝트의 이름 목록. 프로젝트 목록 아래 한 칸에 이름만 보인다
    other_projects = Column(JSON, nullable=False, default=list)
...
class PortfolioProject(Base):
    ...
    tech_stack / tags 와 두 절(highlights · improvements)은 항상 프로젝트와 함께
    ...
    - tech_stack:   ["FastAPI", "React", "OpenAI", ...]   (외부 연동 포함)
    - tags:         ["챗봇", "RAG", ...]
    ...
    # 운영 지표 [{"value": "13곳", "label": "도입 기관"}]. 셀 수 있는 값만 넣고, 누적 수치는 내림해 "+" 로 적는다
