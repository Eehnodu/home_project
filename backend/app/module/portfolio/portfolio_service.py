...
# 제목 + 설명 구조의 두 절. 항목은 {"title", "body"} 이고 개수 제한은 두지 않는다
PROJECT_POINT_FIELDS = ("highlights", "improvements")
POINT_TITLE_MAX = 100
POINT_BODY_MAX = 600
...
class PortfolioService:
    ...
    async def update_info(self, request: Request) -> dict:
        ...
        fields = {
            ...
            "other_projects": self._string_list(body.get("other_projects"), "other_projects", 100),
        }
    ...
    def _to_dict(info: PortfolioInfo) -> dict:
        return {
            ...
            "other_projects": info.other_projects or [],
        }
    ...
    async def save_project(self, request: Request) -> dict:
        ...
        fields["metrics"] = self._metrics(body.get("metrics"))
    ...
    def _metrics(raw: Any) -> list[dict]:
        """운영 지표 [{value, label}]. 값과 설명이 모두 있는 항목만 남긴다"""
        if raw is None:
            return []
        if not isinstance(raw, list):
            fail("metrics 는 배열이어야 합니다.")
        items: list[dict] = []
        for one in raw:
            if not isinstance(one, dict):
                fail("metrics 항목은 값과 설명이어야 합니다.")
            value = str(one.get("value") or "").strip()[:30]
            label = str(one.get("label") or "").strip()[:50]
            if value and label:
