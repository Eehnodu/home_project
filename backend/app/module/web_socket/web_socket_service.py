# 역할: WebSocket 연결 하나의 수명 — 방 등록 · 메시지 루프 · 종료 정리
...
class WebSocketService:
    ...
    async def init_state(self, websocket: WebSocket):
        # 1. 쿼리스트링의 room_id. 없으면 공용 lobby
        room_id = websocket.query_params.get("room_id", "lobby")

        # 2. with_login_web_socket / without_login_web_socket 데코레이터가 심어 둔 사용자 정보
