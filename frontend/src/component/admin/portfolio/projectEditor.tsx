...
import type { PortfolioProject, ProjectMetric, ProjectPoint } from "@/types/portfolio";
...
/* 핵심 · 문제와 개선은 "제목 | 설명" 한 줄에 한 항목으로 편집한다.
...
/* 지표는 "값 | 설명" 한 줄에 하나 */
const metricsToText = (metrics: ProjectMetric[]) =>
  metrics.map((one) => `${one.value}${POINT_SEPARATOR}${one.label}`).join("\n");

const textToMetrics = (text: string): ProjectMetric[] =>
  text
    .split("\n")
...
const ProjectEditor = ({
  ...
  /* 두 절과 지표는 줄 단위 텍스트로 편집한다. 배열 ↔ 텍스트 변환은 열고 저장할 때만 */
  ...
  useEffect(() => {
    ...
    setMetricText(metricsToText(next.metrics));
  }, [open, project]);
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      metrics: textToMetrics(metricText),
    });
  };
    ...
    >
      <div className="flex flex-col gap-5 py-1">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <Field label="지표" hint="한 줄에 하나 · 값 | 설명 (예: 13곳 | 도입 기관)">
            <TextareaBox value={metricText} onChange={setMetricText} rows={7} />
          </Field>
        </div>

        {/* 칩이 길게 늘어나는 필드라 열로 나누지 않고 한 줄씩 가로 전체를 쓴다 */}
        <div className="flex flex-col gap-4">
          <Field label="기술 스택" hint="Enter 로 추가 · 외부 연동도 여기에">
            <ChipInput
